...
def toGeopandas(ContourData, inunits=None):
    ...
    # matplotlib < 3.10 exposed ContourSet.collections (list of PathCollections,
    # one per level).  In 3.10+ ContourSet inherits from Collection directly and
    # collections was removed.  Use get_paths() on the ContourSet itself instead,
    # which returns one Path per level-interval (shell + holes encoded as sub-paths).
