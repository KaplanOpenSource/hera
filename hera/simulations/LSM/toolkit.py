...
class LSMToolkit(toolkit.abstractToolkit):
    ...
    def getTemplateByID(self,simulationid):
        """
            Get a template by its document id

        Parameters
        ----------
        simulationid: str
            The id of the tempalte document.

        Returns
        -------
            The LSMTemplate
        """
        doc = self.getDocumentByID(simulationid)
        return LSMTemplate(doc,self.projectName)
